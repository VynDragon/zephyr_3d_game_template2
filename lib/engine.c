...
L3_Model3D engine_global_models[L3_MAX_MODELS];
L3_Billboard engine_global_billboards[L3_MAX_BILLBOARDS];
L3_Scene engine_global_scene;
...
void L3_computeModelNormals(L3_Model3D *model, L3_Unit *dst,
  int8_t transformNormals)
{
  ...
  L3_makeWorldMatrix(model->transform,m);

  if (transformNormals)
    for (L3_Index i = 0; i < model->model->vertexCount * 3; i += 3)
    {
      n.x = dst[i];
      n.y = dst[i + 1];
      n.z = dst[i + 2];
...
void _L3_projectVertex(const L3_Model3D *model, L3_Index triangleIndex,
...
{
  _L3_projectVertex(model,triangleIndex,0,matrix,&(transformed[0]));
  _L3_projectVertex(model,triangleIndex,1,matrix,&(transformed[1]));
  _L3_projectVertex(model,triangleIndex,2,matrix,&(transformed[2]));
...
uint32_t L3_drawScene(L3_Scene scene)
{
  ...
  const L3_Model3D *model;
  L3_Index modelIndex, triangleIndex, billboardIndex;
  ...
  {
    L3_makeWorldMatrix(scene.billboards[billboardIndex].transform,matFinal);
    ...
    L3_BILLBOARD_FUNCTION(*transformed, &(scene.billboards[billboardIndex]));
  }
  ...
  for (modelIndex = 0; modelIndex < scene.modelCount; ++modelIndex)
  {
    ...
    if (scene.models[modelIndex].customTransformMatrix == 0)
      L3_makeWorldMatrix(scene.models[modelIndex].transform,matFinal);
    else
    {
      L3_Mat4 *m = scene.models[modelIndex].customTransformMatrix;
    ...
    L3_Index triangleCount = scene.models[modelIndex].model->triangleCount;
    ...
    while (triangleIndex < triangleCount)
    {
      ...
      _L3_projectTriangle(model,triangleIndex,matFinal,
      ...
      {
        if (L3_TRIANGLE_FUNCTION(transformed[0],transformed[1],transformed[2],modelIndex,triangleIndex))
        {
#if L3_SORT == 0
        // without sorting draw right away
        L3_drawTriangle(transformed[0],transformed[1],transformed[2],modelIndex,
          triangleIndex);
        ...
        L3_sortArray[L3_sortArrayLength].modelIndex = modelIndex;
  ...
  {
    modelIndex = L3_sortArray[i].modelIndex;
    ...
    if (modelIndex != previousModel)
    {
      ...
      L3_makeWorldMatrix(model->transform,matFinal);
      L3_mat4Xmat4(matFinal,matCamera);
      previousModel = modelIndex;
    }
    ...
    _L3_projectTriangle(model,triangleIndex,matFinal,scene.camera.focalLength,
      transformed);
...
{
	float depthmul = 1.0;
	L3_Model3D *model = &(engine_global_scene.models[p->modelIndex]);
	...
	if (L3_zephyr_putpixel_current_render_mode & L3_VISIBLE_TEXTURED) {
		...
		const L3_Unit *uvs = &(model->model->triangleUVs[p->triangleIndex * 6]);
		...
		if (likely(0 < p->x && L3_RESOLUTION_X > p->x && 0 < p->y && L3_RESOLUTION_Y > p->y))
			L3_video_buffer[p->x + p->y * L3_RESOLUTION_X] = model->model->triangleTextures[model->model->triangleTextureIndex[p->triangleIndex]][(uv[0] >> 0) + (uv[1] >> 0) * L3_TEXTURE_WH] * depthmul;

	} else if (L3_zephyr_putpixel_current_render_mode & L3_VISIBLE_SOLID) {
		if (likely(0 < p->x && L3_RESOLUTION_X > p->x && 0 < p->y && L3_RESOLUTION_Y > p->y))
			L3_video_buffer[p->x + p->y * L3_RESOLUTION_X] = 255 * depthmul;
	}
}
...
{
	L3_zephyr_putpixel_current_render_mode = engine_global_scene.models[modelIndex].config.visible;
...
inline int zephyr_drawbillboard(L3_Vec4 point, L3_Billboard *billboard)
{
	if (!L3_zTest(point.x,point.y,point.z))
		return 0;

	int focal = L3_SCENE.camera.focalLength != 0 ? L3_SCENE.camera.focalLength : 1;
	float scale_x = ((float)(billboard->transform.scale.x * focal) / (float)point.z) * ((float)billboard->scale/0x4000);
	float scale_y = ((float)(billboard->transform.scale.y * focal) / (float)point.z) * ((float)billboard->scale/0x4000);
	...
	for (int i = MAX(0, startx); i < endx; i++) {
		...
		for (int j = MAX(0, starty); j < endy; j++) {
			...
			L3_video_buffer[j * L3_RESOLUTION_X + i] = billboard->texture[m + n * billboard->width];
			#if L3_Z_BUFFER
