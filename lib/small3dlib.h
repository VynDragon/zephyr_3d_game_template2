...
/*
  ...
  The original author has been purged from this document due to extreme disagreement
...
uint32_t S3L_drawScene(S3L_Scene scene)
{
  ...
  for (modelIndex = 0; modelIndex < scene.modelCount; ++modelIndex)
  {
    ...
    while (triangleIndex < triangleCount)
    {
      ...
      {
        if (S3L_TRIANGLE_FUNCTION(transformed[0],transformed[1],transformed[2],modelIndex,triangleIndex))
        {
#if S3L_SORT == 0
