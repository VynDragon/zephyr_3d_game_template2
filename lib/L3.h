...
#define L3_MAX_MODELS 0x7F
#define L3_MAX_BILLBOARDS 0x7F

#define L3_SCENE		engine_global_scene
#define L3_MODELS		engine_global_models
#define L3_BILLBOARDS	engine_global_billboards
...
3: Perform both geometrical and barycentric correction of triangle crossing
	...
	correct rendering. */
#define L3_NEAR_CROSS_STRATEGY 0
...
{
  L3_Transform3D transform;
  L3_Mat4 *customTransformMatrix; /**< This can be used to override the
                                     transform (if != 0) with a custom
                                     transform matrix, which is more
                                     general. */
  L3_DrawConfig config;
  const L3_Model3D_Model *model;
} L3_Model3D;                ///< Represents a 3D model.
...
typedef struct
{
  ...
  L3_Unit barycentric[3]; /**< Barycentric coords correspond to the three
                              ...
                              L3_FRACTIONS_PER_UNIT. */
  L3_Index modelIndex;    ///< Model index within the scene.
...
int L3_BILLBOARD_FUNCTION(L3_Vec4 point, L3_Billboard *billboard);
...
#define INSTANCIATE_MODEL(name, model) L3_Model3D name = {	\
...
extern L3_Model3D engine_global_models[L3_MAX_MODELS];
extern L3_Billboard engine_global_billboards[L3_MAX_BILLBOARDS];
